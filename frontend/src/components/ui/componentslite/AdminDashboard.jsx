        ...
        <div className="space-y-8 sm:space-y-10">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 pt-2">
                ...
                <div className="lg:col-span-2 bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden">
                    <h2 className="text-xl font-bold text-gray-800 p-4 sm:p-6 border-b border-gray-100">
                    ...
                    <ul className="divide-y divide-gray-100">
                        {recentActivities.map((activity) => (
                            <li key={activity.id} className="p-4 sm:p-5 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 hover:bg-gray-50 transition duration-150">
                                <div className="flex items-center gap-3">
                                    <Activity className={`w-5 h-5 flex-shrink-0 ${activity.color}`} />
                    ...
                    <div className="p-4 text-center border-t border-gray-100">
                        <button className={`text-sm ${textViolet} hover:underline font-medium`}>View Full Logs</button>
                    </div>
                </div>
            ...
            <header className="bg-white border-b border-gray-200 shadow-lg sticky top-0 z-10">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    ...
                    <div className="py-4">
                        ...
                        <p className="text-sm text-gray-500 mt-1">Welcome, <span className="font-semibold text-gray-700">{adminName}</span>. Manage your system effectively.</p>
                    </div>

                    {/* Navigation Tabs (Scrollable on smaller mobile devices) */}
                    <nav className="flex space-x-6 sm:space-x-8 pt-1 overflow-x-auto scrollbar-none">
                        {navTabs.map((tab) => (
                            <button
                                key={tab.key}
                                onClick={() => setActiveTab(tab.key)}
                                className={`flex items-center gap-2 pb-3 text-sm font-semibold whitespace-nowrap transition-colors duration-200 ease-in-out flex-shrink-0
                                    ${activeTab === tab.key 
